...
impl IoObject for ExampleIo {
    ...
    fn redirect(
        &mut self,
        function: Vec<String>,
        value: smx::value::Value,
        _amb: &mut smx::value::Ambient,
    ) -> smx::eval::EvalResult<smx::value::Value> {
        if function == ["wait"] {
            if let smx::value::Value::Num(n) = value {
                ...
                Err(eval_error!(WrongTypes(
                    function.join("."),
                    smx::value::PatternType::Number,
                    value
                )))
            }
        } else {
            Err(eval_error!(VariableDoesNotExists(format!(
...
fn test_io_object() {
    let mut amb = smx::val!(ambient);
    amb.add_custom_resource(Arc::new(Mutex::new(ExampleIo)));
    ...
    assert!(
        ...
        "Expected to wait at least 1 second"
    );
    ...
    assert!(
        ...
        "Expected to wait at least 1.5 seconds"
    );
    assert_eq!(res, smx::val!());
}
